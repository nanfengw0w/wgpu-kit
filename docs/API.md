...
Measures real GPU milliseconds spent inside `fn` (timestamp-query; Chrome/Edge).
Rejects with `UsageError` when the device lacks the feature.
...
rebuilds the context** when the GPU device is lost.
...
Sizes the backing store to `clientSize x min(dpr, dprCap)`. Returns whether
the size changed.
