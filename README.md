...
**[API 参考(学习文档)](docs/API.md)** · [基准测试](docs/benchmarks.md) · [变更日志](CHANGELOG.md)

![playground](docs/assets/velocity-final-66000.png)
...
完整数据、复现命令与方法学说明:[docs/benchmarks.md](docs/benchmarks.md)。
...
这不是一个"写完就算"的仓库:每个阶段完成后,由无头浏览器在真实 GPU 上运行结构化探针验证,
验证日志、路径对比、事故复盘记录于开发仓库(未随包发布)。**物理与邻域算法解耦**有常驻回归:
同种子 300 帧,grid 与 tiled 的结构统计偏差阈值 25%/35%,超限即打回。
