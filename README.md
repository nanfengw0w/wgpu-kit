...
- **rAF throughput** — `tick()` per animation frame, CPU/GPU pipelined. This is
  the fps you actually see in the playground (probe: append `?verify=10` to any
  playground URL — it self-reports fps and GPU errors).
...
| --- | --- | --- | --- |
| particles end-to-end | 200,000 @ ~120fps · 66,000 @ ~144fps | rAF | RTX 4060 Laptop, playground probe |
| particle compute (grid), sync | 16k → 200k: 3 → 30 ms/frame | sync | `npm run bench` → docs/BENCHMARK.md |
| neighborhood algorithms | grid ~O(N), 8.5× faster than brute force at 66k | sync A/B | same-session |
| bundle size | core gzip ~10kB (all entries share one context) | — | enforced by `npm run build` |
